...
void evdev_read(lv_indev_drv_t * drv, lv_indev_data_t * data)
{
	...
	if(drv->user_data){
		user_data = (uint32_t *)drv->user_data;
	}
	...
	//LV_LOG_WARN("evdev_root_x: %d evdev_root_y: %d !", evdev_root_x, evdev_root_y);

	if((evdev_root_x != 0)&&(evdev_root_y != 0))
	{
		if((evdev_x != evdev_root_x)&&(evdev_y != evdev_root_y))
		{
			evdev_x = evdev_root_x;
			evdev_y = evdev_root_y;
			
			//LV_LOG_WARN("evdev_root_x && evdev_root_y");
			
			if(user_data)
			{
				//LV_LOG_WARN("user_data = 0");
				*user_data = 0;
			}
		}
	}
